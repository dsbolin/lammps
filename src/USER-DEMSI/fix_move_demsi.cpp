...
FixMoveDemsi::FixMoveDemsi(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg),
  ...
  displace(NULL), velocity(NULL)
{
  ...
  // toriginal = initial orientation of demsi elements
...
void FixMoveDemsi::initial_integrate(int /*vflag*/)
{
  int flag;
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        if (extra_flag) {
          ...
          // theta for lines, demsi elements

	  theta_new = fmod(toriginal[i]+arg,MY_2PI);
          orientation[i] = theta_new;
...
void FixMoveDemsi::final_integrate()
{
  double dtfm;
...
void FixMoveDemsi::initial_integrate_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixMoveDemsi::final_integrate_respa(int ilevel, int /*iloop*/)
{
...
double FixMoveDemsi::memory_usage()
{
...
void FixMoveDemsi::write_restart(FILE *fp)
{
  int n = 0;
  double list[1];
  list[n++] = time_origin;
...
void FixMoveDemsi::restart(char *buf)
{
  int n = 0;
...
void FixMoveDemsi::grow_arrays(int nmax)
{
  memory->grow(xoriginal,nmax,3,"move:xoriginal");
  memory->grow(toriginal,nmax,"move:toriginal");
...
void FixMoveDemsi::copy_arrays(int i, int j, int /*delflag*/)
{
  xoriginal[j][0] = xoriginal[i][0];
  xoriginal[j][1] = xoriginal[i][1];
  xoriginal[j][2] = xoriginal[i][2];
  toriginal[j] = toriginal[i];
  if (quat_flag) {
...
void FixMoveDemsi::set_arrays(int i)
{
  double theta;
  double *quat;
...
int FixMoveDemsi::pack_exchange(int i, double *buf)
{
  int n = 0;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
...
int FixMoveDemsi::unpack_exchange(int nlocal, double *buf)
{
  int n = 0;
  xoriginal[nlocal][0] = buf[n++];
  xoriginal[nlocal][1] = buf[n++];
  xoriginal[nlocal][2] = buf[n++];
  toriginal[nlocal] = buf[n++];
  if (quat_flag) {
...
int FixMoveDemsi::pack_restart(int i, double *buf)
{
  int n = 1;
  buf[n++] = xoriginal[i][0];
  buf[n++] = xoriginal[i][1];
...
void FixMoveDemsi::unpack_restart(int nlocal, int nth)
{
  ...
  toriginal[nlocal] = extra[nlocal][m++];
  if (quat_flag) {
...
int FixMoveDemsi::maxsize_restart()
{
  return nrestart;
}
...
int FixMoveDemsi::size_restart(int /*nlocal*/)
{
  return nrestart;
}
